; RUN: llc -march=mips64 -relocation-model=static -mabi=n32 < %s | FileCheck --check-prefixes=ALL,N32 %s
; RUN: llc -march=mips64el -relocation-model=static -mabi=n32 < %s | FileCheck --check-prefixes=ALL,N32 %s

; RUN: llc -march=mips64 -relocation-model=static -mabi=n64 < %s | FileCheck --check-prefixes=ALL,N64 %s
; RUN: llc -march=mips64el -relocation-model=static -mabi=n64 < %s | FileCheck --check-prefixes=ALL,N64 %s
