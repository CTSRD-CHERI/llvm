; RUN: llc -march=mips64 -mcpu=mips64 -mabi=n32 -relocation-model=pic < \
; RUN:     %s | FileCheck %s -check-prefixes=ALL,N32,HARD-FLOAT
; RUN: llc -march=mips64el -mcpu=mips64 -mabi=n32 -relocation-model=pic < \
; RUN:     %s | FileCheck %s -check-prefixes=ALL,N32,HARD-FLOAT

; RUN: llc -march=mips64 -mcpu=mips64 -mabi=n64 -relocation-model=pic < \
; RUN:     %s | FileCheck %s -check-prefixes=ALL,N64,HARD-FLOAT
; RUN: llc -march=mips64el -mcpu=mips64 -mabi=n64 -relocation-model=pic \
; RUN:     < %s | FileCheck %s -check-prefixes=ALL,N64,HARD-FLOAT

; RUN: llc -march=mips64 -mcpu=mips64 -mattr=+soft-float -mabi=n32 \
; RUN:     -relocation-model=pic < %s | FileCheck %s -check-prefixes=ALL,N32,SOFT-FLOAT
; RUN: llc -march=mips64el -mcpu=mips64 -mattr=+soft-float -mabi=n32 \
; RUN:     -relocation-model=pic < %s | FileCheck %s -check-prefixes=ALL,N32,SOFT-FLOAT

; RUN: llc -march=mips64 -mcpu=mips64 -mattr=+soft-float -mabi=n64 < %s \
; RUN:     -relocation-model=pic | FileCheck %s \
; RUN:     -check-prefixes=ALL,N64,SOFT-FLOAT
; RUN: llc -march=mips64el -mcpu=mips64 -mattr=+soft-float -mabi=n64 < \
