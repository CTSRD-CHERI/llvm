; RUN: llc  < %s -march=mips64el -mcpu=mips4 -mabi=n64 -relocation-model=pic | FileCheck %s -check-prefix=CHECK-N64
; RUN: llc  < %s -march=mips64el -mcpu=mips4 -mabi=n32 -relocation-model=pic | FileCheck %s -check-prefix=CHECK-N32
; RUN: llc  < %s -march=mips64el -mcpu=mips64 -mabi=n64 -relocation-model=pic | FileCheck %s -check-prefix=CHECK-N64
; RUN: llc  < %s -march=mips64el -mcpu=mips64 -mabi=n32 -relocation-model=pic | FileCheck %s -check-prefix=CHECK-N32
